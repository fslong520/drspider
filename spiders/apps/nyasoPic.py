...
class Nyaso(object):
    def __init__(self, *args, **kwargs):
        ...
        req = requests.get('https://pic.nyaso.com/pixivday/', headers=self.header)
    ...
    async def getnysasoWallpaperPic(self, page, loop):
        ...
        def loop1(dataJson):
            for i in dataJson['lists']:
                self.getPic(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(
                    __file__))), 'img/nyasoPic/wallPaper'), i)
        ...
        if 'wallPaper' in basePath:
            try:
                ...
                image.save(os.path.join(
                    basePath, picDict['title'])+'.'+picDict['down'].split('/')[-2])
            except:
                traceback.print_exc()

        else:
            try:
                print(picDict['title']+':  '+picDict['preview'].split('?')[0])
                req = requests.get(picDict['preview'].split('?')[0], headers=self.header)
                # 将下载到的数据转换成bytesIo之后用image打开
                image = Image.open(BytesIO(req.content))
                image.save(os.path.join(
                    basePath, picDict['title'])+'.jpg')
            except:
                traceback.print_exc()
